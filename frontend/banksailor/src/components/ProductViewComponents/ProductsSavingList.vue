<template>
  ...
  <div v-if="isReverse">
    <ProductsSavingItem v-for="product in sortedReverseProducts" :key="product.id" :product="product"/>
  </div>
  <div v-else>
    <ProductsSavingItem v-for="product in sortedProducts" :key="product.id" :product="product"/>
  </div>
</template>
...
const sortedProducts = computed(() => {
  ...
  if (sortValue.value) {
    ...
    if (props.selectedBank === '전체') {
      ...
      return props.selectedBank
      ? store.savings.filter((product) => product.kor_co_nm === props.selectedBank)
      : store.savings;
    }
  }
});
